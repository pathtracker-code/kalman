...
files = glob(os.path.join("single_dist_results", "*.mat"))
...
for vidx, f in enumerate(files):
    data = io.loadmat(f)
    vidmat = io.loadmat(os.path.join("single_mat", "vid_{}.mat".format(int(f.split("_")[-1].split(".")[0]))))
...
for vidx in tqdm(range(len(files))):
    vidmat = io.loadmat(os.path.join("single_mat", "vid_{}.mat".format(vidx)))
    video = vidmat["video"]
    ...
    # plt.savefig(os.path.join("single_ex", "vid_frames_{}.pdf".format(vidx)))
    # # plt.show()
    # plt.close(f)
    ...
    plt.savefig(os.path.join("single_ex", "vid_tracks_{}.pdf".format(vidx)))
